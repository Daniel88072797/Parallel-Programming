/*shared memory、Coalesced Memory Access、sequential addressing(handle bank conflict、handle warp divergent)、curand()*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
__global__ void kernel1(double *V, double *pop, double *pbest, double *gbest, curandState *state, double *fitness, double *fitnesspbest, double *outputMax){   //改用cuRand
...
{       
	...
	// Copy Data to be Calculated
        ...
        for(int i = 0; i < gen; i++){
                ...
                kernel1<<<num_block ,num_threads, num_threads * sizeof(double)>>>(d_V, d_pop, d_pbest, d_gbest, state, d_fitness, d_fitnesspbest, d_outputMax);
