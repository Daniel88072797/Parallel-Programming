...
#define blocks ceil(((float)sizepop) / 1024)
// #define func(a) (((a)*(a)*(a)))
...
__global__ void kernel1(double *V, double *pop, double *pbest, double *gbest, double random1, double random2, double *fitness, double *fitnesspbest, double *outputMax){   //改用cuRand
        ...
        double sdata[1024];
        /*move fitnesspbest to local shared data*/
        ...
        if(tid == 0) outputMax[blockIdx.x] = sdata[0]; 
...
__global__ void kernel2(double *outputMax){   //利用reduction
        ...
        for(int s = 1; s < blockDim.x; s *= 2){
                if(tid % (2 * s) == 0){
                        ...
                        outputMax[tid] = max(outputMax[tid], outputMax[tid + s]);
                }
                __syncthreads();
        }
...
{       
	...
	// Copy Data to be Calculated
        ...
        for(int i = 0; i < gen; i++){
                ...
                kernel1<<<num_block ,num_threads>>>(d_V, d_pop, d_pbest, d_gbest, r, r, d_fitness, d_fitnesspbest, d_outputMax);
                kernel2<<<1, num_block>>>(d_outputMax);
